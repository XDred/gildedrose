        ...
        foreach ($this->items as $item) {
            ...
            if($item->sell_in < 0){ // если продукт просрочен
                if($cur_setting['expired'] === '=0'){ // если в настройках указано что просрочку сразу устанавливаем в 0
                    ...
                    if(($item->quality+$cur_setting['expired'] >=0) && ($item->quality+$cur_setting['expired'] <=50)){ // если итоговый параметр не выбивается за границы качества
                        $item->quality = $item->quality + $cur_setting['expired']; // добавляем сколько нужно
                    }
                }
                ...
                if(isset($cur_setting['days'])){ // проверяем, есть ли у него особый параметр 
                    ...
                    foreach ($days as $day){
                        if($item->sell_in < $day) $cur_quality = $cur_setting['days'][$day]; // и ищем нужную настройку
                    }                        
                    if(($item->quality+$cur_quality >=0) && ($item->quality+$cur_quality <=50)){ // если итоговый параметр не выбивается за границы качества 
                        $item->quality = $item->quality + $cur_quality; // добавляем сколько нужно
                    }        
                }else{ // если нет особого параметра
                    if(($item->quality+$cur_setting['quality'] >=0) && ($item->quality+$cur_setting['quality'] <=50)){ // если итоговый параметр не выбивается за границы качества 
                        $item->quality = $item->quality + $cur_setting['quality']; // добавляем сколько нужно
                    }
                }
